...
class ChatModelPipeline:
    def __init__(self, model_configs: ModelConfigs):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(model_configs.model_name_or_path)

        self.system_prompt = model_configs.system_prompt
    ...
    def generate(
        ...
        max_new_tokens = self.max_seq_len - self.system_prompt_len - len(model_input)

        with torch.inference_mode():
            output = self.model.generate(
                **model_input,
                temperature=self.model_configs.temperature,
                top_p=self.model_configs.top_p,
                top_k=self.model_configs.top_k,
                repetition_penalty=self.model_configs.repetition_penalty,
